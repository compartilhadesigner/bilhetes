...
for website in WEBSITES:
    Logger.debug(f"Iniciando a extração do site {website['name']}")
    ...
    if 'isSA' in website and website['isSA']:
        sa = SAWebsite(webdriver=driver, websiteName=website['name'])
        sa.get_bet_data()

    if website['name'] == 'B2X':
        b2x = B2XWebsite(webdriver=driver, websiteName=website['name'])
        b2x.get_bet_data()

    if website['name'] == 'CENTRAL BET':
        cb = CentralBet(webdriver=driver, websiteName=website['name'])
        cb.get_bet_data()

    if website['name'] == 'SYMBET':
        sb = SymBet(webdriver=driver, websiteName=website['name'])
        sb.get_bet_data()

    if website['name'] == 'SPORTBET365':
        s365 = SportBet365(webdriver=driver, websiteName=website['name'])
        s365.get_bet_data()

    if website['name'] == '365BR':
        b365 = BR365Bet(webdriver=driver, websiteName=website['name'])
        b365.get_bet_data()
